...
from dataset.dynamic_dataloader1 import RestrictedDataset
from torch.utils.data import DataLoader

import json
from utils.std_metric import get_segmentation_mask_uncertainty
...
dir_img_test = '/dataset.local/all/hangd/dynamic_data/full/data_test/imgs/'
dir_mask_test = '/dataset.local/all/hangd/dynamic_data/full/data_test/masks/'
...
def train_net(
    ...
    net = PAN()
    ckpt_path = "/dataset.local/all/hangd/v1/uncertainty1/best_CP_epoch29_one32th_.pth"
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_pool, desc='STD calculating', unit='batch', leave=False) as pbar:
            for ind, batch in enumerate(tqdm(pool_loader)):
                ...
                is_selected = False
                # TODO
                # Using some constraint here to select dataset
                ...
                #         count += 1
                pbar.update()
    # print("The # of selected images = ", count)
    # for ind, val in enumerate(std):
    #     writer.add_scalar("std in pool - case one32nd dataset", val, ind)
    #
    ...
    for i in top_100img:
        add_image_id_to_pool(i, "../tmp/data_one32nd_std.json")
